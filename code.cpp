...
if((int)roots.size()<n){int k=__builtin_ctz(roots.size());roots.resize(n);while((1<<k)<n){double ang=3.14159265358979323846264338327950288/(1<<k);for(int i=1<<(k-1);i<(1<<k);++i){roots[2*i]=roots[i];roots[2*i+1]=complex<double>(cos(ang*(2*i+1-(1<<k))),sin(ang*(2*i+1-(1<<k))));}++k;}}
for(int i=0;i<n;i++)if(i<rev[i])std::swap(a[i],a[rev[i]]);for(int len=1;len<n;len<<=1){for(int i=0;i<n;i+=2*len){for(int j=0;j<len;++j){complex<double> u=a[i+j],v=a[i+j+len]*(inv?std::conj(roots[len+j]):roots[len+j]);a[i+j]=u+v;a[i+j+len]=u-v;}}}if(inv){for(auto &x:a)x/=n;for(int i=1;i<n/2;i++)std::swap(a[i],a[n-i]);}}
static void conv(const vector<int>&A,const vector<int>&B,vector<long long>&res){int n=1;while(n<(int)A.size()+(int)B.size())n<<=1;vector<complex<double>> fa(n),fb(n);for(size_t i=0;i<A.size();++i)fa[i]=A[i];for(size_t i=0;i<B.size();++i)fb[i]=B[i];fft(fa,false);fft(fb,false);for(int i=0;i<n;i++)fa[i]*=fb[i];fft(fa,true);res.assign(n,0);long long carry=0;for(int i=0;i<n;i++){long long t=(long long)llround(fa[i].real())+carry;res[i]=t%BASE;carry=t/BASE;}while(carry){res.push_back(carry%BASE);carry/=BASE;}}
...
int2048 int2048::mulAbs(const int2048&a,const int2048&b){int2048 r;if(a.s==0||b.s==0){r.s=0;return r;}if(a.d.size()<64||b.d.size()<64){r.s=1;r.d.assign(a.d.size()+b.d.size(),0);for(size_t i=0;i<a.d.size();++i){long long carry=0;for(size_t j=0;j<b.d.size()||carry;++j){long long cur=r.d[i+j]+carry+(long long)a.d[i]*(j<b.d.size()?b.d[j]:0);r.d[i+j]=(int)(cur%BASE);carry=cur/BASE;}}r.trim();return r;}vector<long long> tmp;conv(a.d,b.d,tmp);r.s=1;r.d.resize(tmp.size());long long carry=0;for(size_t i=0;i<tmp.size();++i){long long cur=tmp[i]+carry;r.d[i]=(int)(cur%BASE);carry=cur/BASE;}while(carry){r.d.push_back((int)(carry%BASE));carry/=BASE;}r.trim();return r;}
