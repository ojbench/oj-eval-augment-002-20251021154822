...
namespace sjtu {
static const int BASE=10000;static const int BD=4;using std::vector;using std::string;using std::istream;using std::ostream;using std::complex;static void fft(std::vector<complex<double>>& a,bool inv){int n=a.size();vector<int> rev(n);for(int i=1,j=0;i<n;i++){int bit=n>>1;for(;j&bit;bit>>=1)j^=bit;j^=bit;rev[i]=j;}for(int i=0;i<n;i++)if(i<rev[i])std::swap(a[i],a[rev[i]]);const double PI=3.14159265358979323846264338327950288;for(int len=1;len<n;len<<=1){double ang=(inv?-1:1)*PI/len;complex<double> wlen(cos(ang),sin(ang));for(int i=0;i<n;i+=2*len){complex<double> w(1,0);for(int j=0;j<len;++j){complex<double> u=a[i+j],v=a[i+j+len]*w;a[i+j]=u+v;a[i+j+len]=u-v;w*=wlen;}}}if(inv){for(auto &x:a)x/=n;}}
static void conv(const vector<int>&A,const vector<int>&B,vector<long long>&res){int n=1;while(n<(int)A.size()+(int)B.size())n<<=1;vector<complex<double>> fa(n),fb(n);for(size_t i=0;i<A.size();++i)fa[i]=A[i];for(size_t i=0;i<B.size();++i)fb[i]=B[i];fft(fa,false);fft(fb,false);for(int i=0;i<n;i++)fa[i]*=fb[i];fft(fa,true);res.assign(n,0);for(int i=0;i<n;i++)res[i]=(long long)llround(fa[i].real());}
